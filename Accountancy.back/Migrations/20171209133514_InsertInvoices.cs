...
namespace Accountancy.Migrations
{
    ...
    public class InsertInvoices : Migration
    {
        private class InvoiceInserter
        {
            ...
            private void CreateInvoice(Company receiver, InvoiceStatus status, decimal? amount, string theirReference, (string name, int amount, decimal price)[] extraLines)
            {
                ...
                _context.Add(new Invoice
                {
                    ...
                    InvoiceLines = GetInvoiceLines(amount, extraLines).ToList()
                });
                _context.SaveChanges();
            }

            private IEnumerable<InvoiceLine> GetInvoiceLines(decimal? amount, (string name, int amount, decimal price)[] extraLines)
            {
                if (amount == null)
                    yield break;
            ...
            public void CreateInvoices(decimal? amountQframe, decimal? amountCronos = null, params (string name, int amount, decimal price)[] extraLinesQframe)
            {
                if (amountQframe.HasValue)
                    CreateInvoice(_qframe, InvoiceStatus.Sent, amountQframe, null, extraLinesQframe);

                if (amountCronos.HasValue)
                    CreateInvoice(_cronos, InvoiceStatus.Sent, amountCronos, ReferenceCronos, new (string name, int amount, decimal price)[0]);
        ...
        private static void InsertAllInvoices(DatabaseContext context, Company nfSoftware, Company qframe, Company cronos)
        {
            ...
            invoiceInserter.CreateInvoices(21.21m, null, new [] { ("Variabele vergoeding 2022", 1, 1009m) });
            invoiceInserter.CreateInvoices(10.28m);
            invoiceInserter.CreateInvoices(22.09m, null, new [] { ("Interne dagen", 4, 0m) });
            invoiceInserter.CreateInvoices(20.94m);
            invoiceInserter.CreateInvoices(20.25m, null, new [] { ("Interne dagen", 4, 0m) });
