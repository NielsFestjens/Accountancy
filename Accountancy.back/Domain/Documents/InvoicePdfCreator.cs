...
namespace Accountancy.Domain.Documents;
...
public class InvoicePdfCreator : IInvoicePdfCreator
{
    ...
    private static void GenerateContent(IElementListener document, PdfContentByte directContent, Invoice invoice)
    {
        ...
        var extraInformationPart = GetExtraInformationPart(invoice);
        if (extraInformationPart != null)
        {
            document.Add(extraInformationPart);
            document.Add(new Paragraph("\n"));
        }
    ...
    private static IElement GetExtraInformationPart(Invoice invoice)
    {
        if (invoice.ExtraInformation == null)
            return null;
        
        return new Paragraph($"Extra informatie:\r\n{invoice.ExtraInformation}", PdfHelper.BaseFont);
    }
